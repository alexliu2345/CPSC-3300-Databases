...
CREATE PROCEDURE SP_PASSENGER_ADD (IN P_NAME_FIRST VARCHAR(25),IN P_NAME_LAST VARCHAR(25)) 
	...
	WHERE 
		NOT EXISTS (SELECT PASSENGER_ID FROM PASSENGER WHERE PASSENGER_NAME_LAST = P_NAME_LAST AND
        PASSENGER_NAME_FIRST = P_NAME_FIRST);
...
IF NOT EXISTS (
	...
	WHERE 
        ...
        -- Added in next line because the procedure was changing the seatrow/column of every passenger on flight
        MANIFEST.PASSENGER_ID = (SELECT PASSENGER_ID FROM PASSENGER WHERE PASSENGER_NAME_FIRST = P_FIRST AND PASSENGER_NAME_LAST = P_LAST) AND
...
*/
	...
	INSERT INTO PASSENGER (PASSENGER_NAME_LAST, PASSENGER_NAME_FIRST) SELECT PASSENGER_NAME_LAST, PASSENGER_NAME_FIRST FROM km_airline.PASSENGER;
	 INSERT INTO PLANE (PLANE_NAME, PLANE_MODEL, PLANE_ROWS, PLANE_COLUMNS) SELECT PLANE_NAME, PLANE_MODEL, PLANE_ROWS, PLANE_COLUMNS FROM km_airline.PLANE;
...
-- Add Plane P1 787 with 40 rows and 8 columns
-- Add Plane P2 777 with 30 rows and 6 columns
-- Add Plane P3 747 with 50 rows and 9 columns
...
-- 6. 
-- Add a flight on the SEA for a 787, LAX for a 777, LHR for a 747
CALL SP_FLIGHT_ADD('SEA','787');
CALL SP_FLIGHT_ADD('LAX', '777');
CALL SP_FLIGHT_ADD('LHR','747');

-- Add Passengers Mike, Masami, Asumi and Jake
...
-- Add the passengers to the flights
...
-- 7. Check that Mike isnot inserted twice in to manifest by doing a select on manifest to check.
...
-- 8. Try to changes seats to put someone into an occupied seat, select from manifest to test
...
-- CALL SP_MANIFEST_ASSIGN_SEAT (1,'SEA', 'Taga','Asumi', 1, 4);




/*
	Confirm data inserted by selecting from Views
*/
SELECT * FROM VIEW_FLIGHT_ALL;
