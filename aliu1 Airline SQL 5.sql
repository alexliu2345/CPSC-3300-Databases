...
*/
	...
	INSERT INTO DESTINATION (DESTINATION_CODE, DESTINATION_NAME) SELECT DESTINATION_CODE, DESTINATION_NAME FROM km_airline.DESTINATION;
...
-- 1. How many people are on the flight to SEA?
	-- Return destination name, destination code, and count of the passengers as NUMBER on the flight to SEA?
	SELECT DESTINATION_NAME, DESTINATION_CODE, COUNT(MANIFEST.FLIGHT_ID) as 'NUMBER'
    FROM MANIFEST
    INNER JOIN FLIGHT ON FLIGHT.FLIGHT_ID = MANIFEST.FLIGHT_ID
    INNER JOIN DESTINATION ON DESTINATION.DESTINATION_ID = FLIGHT.DESTINATION_ID
	WHERE DESTINATION_CODE = 'SEA'
	GROUP BY DESTINATION_NAME, DESTINATION_CODE, MANIFEST.FLIGHT_ID;
	        
-- 2. What are the most popular flights including Destination, Code, and Count
	-- Destination and Count of passengers per all destinations ordered by the count highest to lowest
	SELECT DESTINATION_NAME, DESTINATION_CODE, COUNT(MANIFEST.FLIGHT_ID) as 'NUMBER'
    FROM MANIFEST
    INNER JOIN FLIGHT ON FLIGHT.FLIGHT_ID = MANIFEST.FLIGHT_ID
    INNER JOIN DESTINATION ON DESTINATION.DESTINATION_ID = FLIGHT.DESTINATION_ID
	GROUP BY MANIFEST.FLIGHT_ID, DESTINATION_NAME, DESTINATION_CODE
	ORDER BY COUNT(MANIFEST.FLIGHT_ID) DESC;
	
-- 3. Who are the top flyers?
	-- Name, and Count of the passenger who fly ordered with most flights first
	SELECT 
		CONCAT(PASSENGER_NAME_LAST,',',PASSENGER_NAME_FIRST) as 'FULLNAME', 
        COUNT(MANIFEST.PASSENGER_ID) as 'NUMBER'
    FROM MANIFEST
    INNER JOIN PASSENGER ON PASSENGER.PASSENGER_ID = MANIFEST.PASSENGER_ID
 	GROUP BY PASSENGER_NAME_LAST, PASSENGER_NAME_FIRST, MANIFEST.PASSENGER_ID
 	ORDER BY 
		COUNT(MANIFEST.PASSENGER_ID) desc;