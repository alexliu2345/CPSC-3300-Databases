...
-- 1
-- What passenger by Full name needs a seat assignment on the flight to Seattle? (Use a view...)
SELECT FULLNAME 
	FROM VIEW_PASSENGERS_UNASSIGNED
	WHERE DESTINATION_CODE = 'SEA';

-- 2
-- Starting from Destination, What destinations Name have no flights?
SELECT DESTINATION.DESTINATION_NAME 
	FROM DESTINATION
	LEFT OUTER JOIN FLIGHT ON DESTINATION.DESTINATION_ID = FLIGHT.DESTINATION_ID
    WHERE FLIGHT.DESTINATION_ID IS NULL;

-- 3
-- Starting from Plane, what Destiantion Code have no flights?    
SELECT DESTINATION.DESTINATION_CODE 
FROM PLANE
	INNER JOIN FLIGHT
    RIGHT OUTER JOIN DESTINATION ON DESTINATION.DESTINATION_ID = FLIGHT.DESTINATION_ID
    WHERE FLIGHT.DESTINATION_ID IS NULL;
 
-- 4
-- What passengers first name have not booked a flight
select PASSENGER_NAME_FIRST from FLIGHT
	INNER JOIN MANIFEST
    INNER JOIN DESTINATION
    INNER JOIN PLANE
    RIGHT OUTER JOIN PASSENGER ON PASSENGER.PASSENGER_ID = MANIFEST.PASSENGER_ID
    WHERE FLIGHT.FLIGHT_ID IS NULL;
